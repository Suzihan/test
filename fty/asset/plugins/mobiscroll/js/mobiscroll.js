...
(function(a, b) {
    ...
    c.presets.scroller.measurement = function(c) {
        function l(a) {
            return Math.max(R, Math.min(M, y ? 0 > a ? Math.ceil(a) : Math.floor(a) : S(Math.round(a - w), E) + w))
        }
        function i(a) {
            return y ? S((Math.abs(a) - Math.abs(l(a))) * x - t, E) + t: 0
        }
        function B(a) {
            var b = l(a),
            c = i(a);
        ...
        function G(a) {
            var b = +a[z],
            c = y ? v(a[I]) / x * (0 > b ? -1 : 1) : 0;
            return (j && "-" == a[0] ? -1 : 1) * (b + c)
        }
        function S(a, b) {
        ...
        function fa(a) {
            h = X(m.min, P, a);
            J = X(m.max, P, a);
            y ? (R = 0 > h ? Math.ceil(h) : Math.floor(h), M = 0 > J ? Math.ceil(J) : Math.floor(J), $ = i(h), d = i(J)) : (R = Math.round(h), M = Math.round(J), M = R + Math.floor((M - R) / E) * E, w = R % E)
        }
        function V(a, b) {
            var c = +R,
            d = +M,
            e = y ? 1 : E,
            g;
            j && (d = Math.abs(c) > Math.abs(d) ? Math.abs(c) : Math.abs(d), c = 0 > c ? 0 : c);
            g = b - ha * e;
            g = g < c ? c: g;
            c = g + 2 * ha * e;
            c = c > d ? d: c;
            if (g !== ea || c !== Y) {
                ga.values = [];
                ga.label = m.wholeText;
                for (u = g; u <= c; u += e) ga.values.push(u);
                W = g;
                ia = c;
                return ! 0
            }
        ...
        function ja(b) {
            if (Z) {
                ...
                L.keys = [];
                L.values = [];
                for (u = -50; 50 > u; u++) d = (u + b) % c,
                e = T[0 > d ? c + d: d],
                d = Math.abs(Math.floor((u + b) / c)),
                L.keys.push(Array(d).join(0 > u + b ? "-": "+") + e),
                L.values.push("." + N(e, O))
            }
        }
        function da(a) {
            return G(a).toFixed(y ? O: 0) + (U ? " " + D[a[Q]] : "")
        }
        var H = a.extend({},
        c.settings),
        m = a.extend(c.settings, n, H),
        H = {},
        ba = [[]],
        r = {},
        ga = {},
        ...
        ia,
        j = m.sign,
        U = m.units && m.units.length,
        P = U ? m.defUnit || m.units[0] : "",
        D = m.unitNames || m.units,
        y = 1 > m.step,
        Z = !1,
        O = y ? Math.max(m.scale, (m.step + "").split(".")[1].length) : 1,
        x = Math.pow(10, O),
        E = Math.round(y ? m.step * x: m.step),
        A,
        q,
        F,
        k = -1,
        ...
        u,
        e = 0;
        c.setVal = function(b, d, e, h, j) {
            c._setVal(a.isArray(b) ? da(b) : b, d, e, h, j)
        };
        c.setValue = function(a, b, d, e, h) {
            c.setVal(a, b, h, e, d)
        };
        fa(P);
        if (j) if (j = !1, U) for (u = 0; u < m.units.length; u++) 0 > X(m.min, P, m.units[u]) && (j = !0);
        else j = 0 > m.min;
        j && (ba[0].push({
            values: ["-", "+"],
            label: m.signText
        ...
        if (y) {
            ...
            L.keys = [];
            L.values = [];
            L.label = m.fractionText;
            for (u = t; u < x; u += E) T.push(u),
            L.keys.push(u),
            L.values.push("." + N(u, O));
            Z = T.length > m.rows;
            I = e++;
        ...
        if (U) {
            ...
            for (u = 0; u < m.units.length; u++) H.keys.push(u),
            H.values.push(D[u]);
            ba[0].push(H)
        }
        Q = e;
        return {
            ...
            onBeforeShow: function() {
                V(F, c._tempWheelArray[z]);
                ja(v(c._tempWheelArray[I]));
                ea = W;
                Y = ia;
                C = true
            },
            ...
            validate: function(e, f, i, n) {
                var l = c._tempWheelArray,
                p, x = [],
                H,
                t,
                A,
                ...
                N = U ? m.units[l[Q]] : "";
                if (j && f === 0) {
                    q = Math.abs(q) * (l[f] === "-" ? -1 : 1);
                    x = Z ? [z, I] : [z]
                }
                if (f === z || f === I && y || q === b || f === b && !C) {
                    q = G(l);
                    F = N
                }
                if (U && f === Q && F !== N || f === b && !C) {
                ...
                q = K(q, h, J);
                t = B(q);
                D = j ? Math.abs(t[1]) : t[1];
                p = j ? l[0] == "-": q < 0;
                l[z] = D;
                p && (t[0] = "-");
                y && (l[I] = t[2]);
                if (f === z || j && f === 0) O = V(N, D);
                f === z && O && x.push(z);
                if (f === I && Z) {
                    ja(T);
                    x.push(I)
                ...
                if (x.length) {
                    ka[f] = setTimeout(function() {
                        C = true;
                        ea = W;
                        Y = ia;
                        c.changeWheel(x, H, f !== b)
                    },
                    f === b ? 0 : i * 1E3);
                    return false
                }
                if (j && f === b) {
                    ...
                    h > 0 && a(".dw-li", w).eq(0).removeClass("dw-v");
                    J < 0 && a(".dw-li", w).eq(1).removeClass("dw-v")
                }
                w = a(".dw-ul", e).eq(z);
                if (j && !f) {
                    a(".dw-li", w).addClass("dw-v");
                    u = a(".dw-li", w).index(a('.dw-li[data-val="' + Math.abs(p ? R: M) + '"]', w));
                    u != -1 && a(".dw-li", w).slice(u + 1).removeClass("dw-v")
                }
                a.each(y ? r: m.invalid,
                function(b, c) {
                    if (j && p) if (c <= 0) c = Math.abs(c);
                    else return;
                    a('.dw-li[data-val="' + S(X(c, P, N), y ? 1 : E) + '"]', w).removeClass("dw-v")
                });
                l[z] = +c.getValidCell(D, w, n).val;
                t[1] = l[z] * (j && p ? -1 : 1);
                if (f !== I && y) {
                    w = a(".dw-ul", e).eq(I);
                    a(".dw-li", w).addClass("dw-v");
                    e = j ? l[0] + v(l[1]) : (q < 0 ? "-": "+") + Math.abs(t[1]);
                    i = (h < 0 ? "-": "+") + Math.abs(R);
                    n = (J < 0 ? "-": "+") + Math.abs(M);
                    e === i && a(".dw-li", w).each(function() {
                        L = v(a(this).attr("data-val")); (p ? L > $: L < $) && a(this).removeClass("dw-v")
                    });
                    e === n && a(".dw-li", w).each(function() {
                        L = v(a(this).attr("data-val")); (p ? L < d: L > d) && a(this).removeClass("dw-v")
                    });
                    a.each(m.invalid,
                    function(b, c) {
                        A = B(X(c, P, N)); (t[0] === A[0] || t[1] === 0 && A[1] === 0 && A[2] === 0) && t[1] === A[1] && a(".dw-li", w).each(function() {
                            v(a(this).attr("data-val")) == A[2] && a(this).removeClass("dw-v")
                        })
                    })
                }
                C = false
